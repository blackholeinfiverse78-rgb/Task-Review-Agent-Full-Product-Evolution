...
**Enforcement**: Requires PENDING_REVIEW state. Idempotent block on re-approvals.

`POST /api/v1/review/reject`
**Purpose**: Transition state to REJECTED.
**Enforcement**: Requires PENDING_REVIEW state.

`POST /api/v1/review/modify`
**Purpose**: Adjust bounded metadata ONLY.
**Enforcement**: Dual-approval needed (authorizer + operator).
